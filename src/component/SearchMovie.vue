    ...
    <div class="card bg-base-100 p-5">
      <div class="form-control">
        ...
        <div class="join mt-3 w-full">
          ...
          <button v-if="change" class="join-item btn btn-primary" @click="$emit('cancel')">
            cancel change
          </button>
        </div>
      </div>
      ...
      <div v-else-if="searchItems.length > 0" class="mt-6">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          <div v-for="item in searchItems" :key="item['#IMDB_ID']"
            ...
            <div class="card card-compact bg-base-100 cursor-pointer shadow-lg transition-shadow hover:shadow-xl h-full"
              @click="selectMovie(item['#IMDB_ID'])">
              <figure class="h-48">
                <img :src="item['#IMG_POSTER'] !== 'N/A'
                  ? item['#IMG_POSTER']
